...
def swap_local(
    ...
    if rank == 0:
        rank = tsr1.shape[2]
    u, _, vh = svd(tsr.reshape(np.prod(shape[:2]), -1), rank, absorb_s)
...
def swap(
    ...
    if dest > start:
        for i in range(start, dest):
            mps[i], mps[i + 1] = swap_local(mps[i], mps[i + 1], rank, absorb_s)
    else:
        for i in range(start, dest, -1):
            mps[i - 1], mps[i] = swap_local(mps[i - 1], mps[i], rank, absorb_s)
    return mps


def trace_target_unitary(
    mps: Sequence[NDArray[np.complex128]],
...
@lru_cache(None)
def basis_state_arr(dim: int, index: int) -> NDArray[np.complex128]:
    arr = np.zeros(dim)
    arr[index] = 1
    return arr
    ...
    for tsr, bit in zip(mps, bitstring):
        ...
        ret.append(
            ...
            np.einsum(
                "ipj,p->ij",
                tsr,
                basis_state_arr(tsr.shape[1], bit),
            ).reshape(tsr.shape[0], 1, tsr.shape[2])
        )
    return ret


def to_dense(mps: Sequence[NDArray[np.complex128]]) -> NDArray[np.complex128]:
    tsr = mps[0]
